...
void LCD_DrawRectToBuffer(uint8_t* buffer, uint32_t x0, uint32_t y0, uint32_t width, uint32_t height, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    if (x0 + width > SCREEN_WIDTH) width = SCREEN_WIDTH - x0;
    if (y0 + height > SCREEN_HEIGHT) height = SCREEN_HEIGHT - y0;
    
    // Exit if rectangle is completely off-screen
    if (width <= 0 || height <= 0) return;
    
    // Try a simpler approach that mimics the screen's physical layout more directly
    // Based on the observation of how LCD_DrawScreenBuffer sets up the window
    
    for (uint32_t j = 0; j < height; j++) {
        for (uint32_t i = 0; i < width; i++) {
            uint32_t screen_x = x0 + i;
            uint32_t screen_y = y0 + j;
            
            // The key insight is understanding how pixels are laid out in memory when
            // passed to LCD_DrawScreenBuffer
            uint32_t index = ((SCREEN_WIDTH - 1 - screen_x) * SCREEN_HEIGHT + screen_y) * 3;
...
{
    // If all colors are the same, use memset (much faster)
    if (r == g && g == b) {
        memset(buffer, r, SCREEN_WIDTH * SCREEN_HEIGHT * 3);
    } else {
        // Otherwise, we need to set each pixel individually with the correct memory layout
        for (uint32_t x = 0; x < SCREEN_WIDTH; x++) {
            for (uint32_t y = 0; y < SCREEN_HEIGHT; y++) {
                uint32_t index = ((SCREEN_WIDTH - 1 - x) * SCREEN_HEIGHT + y) * 3;
...
// Draw a circle to buffer
void LCD_DrawCircleToBuffer(uint8_t* buffer, int32_t centerX, int32_t centerY, int32_t radius, uint8_t r, uint8_t g, uint8_t b)
{
    // Check boundaries to avoid unnecessary calculations
    int32_t minX = centerX - radius;
    int32_t maxX = centerX + radius;
    int32_t minY = centerY - radius;
    int32_t maxY = centerY + radius;
    ...
    // Draw the circle using the same coordinate transformation as the rectangle function
    for (int32_t y = minY; y <= maxY; y++) {
        for (int32_t x = minX; x <= maxX; x++) {
            ...
            if (dx*dx + dy*dy <= radius*radius) {
                // Apply the same transformation as in LCD_DrawRectToBuffer
                uint32_t index = ((SCREEN_WIDTH - 1 - x) * SCREEN_HEIGHT + y) * 3;
